...
template <typename T>
constexpr auto MagNum2PNMTYPE(T magicNum)
{
...
template <typename T>
constexpr auto PNMTYPE2MagNum(T type)
{
...
public:
	...
	pnm_io::PNM_STATE ConvertFormat(PNM *src, PNM *dst, std::vector<std::float_t> const *pa );
	pnm_io::PNM_STATE ConvertFormat(PNM *f, PNMTYPE type, std::vector<std::float_t> const *pa);
